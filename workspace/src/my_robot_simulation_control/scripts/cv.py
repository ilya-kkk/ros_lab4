...
class WhiteRegionDetector:
    def __init__(self):
        ...
        self.image_publisher = rospy.Publisher("/diff_drive_robot/camera1/white_region", Image, queue_size=10)
    ...
    def camera_cb(self, data):
        try:
            ...
            _, binary_image = cv2.threshold(gray_frame, 175, 255, cv2.THRESH_BINARY)
            ...
            for contour in contours:
                if cv2.contourArea(contour) > 100:  # Игнорируем слишком маленькие области
                    ...
                    if M["m00"] > 0:  # Проверяем, что площадь ненулевая
                        ...
                        cv2.line(frame, center_image, (cX, cY), (0, 255, 0), 2)  # Зеленая линия

                        # Вычисляем расстояние по оси X
                        distance_x = cX - center_image[0]

                        # Публикуем значение расстояния по X
                        self.value_publisher.publish(distance_x)

                        # Отмечаем, что белая область найдена
